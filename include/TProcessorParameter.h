   ...
   public:
      ...
      virtual TString Type() const
      {
         ...
         if (typeid(T) == typeid(Int_t))
            return "Int_t";
         if (typeid(T) == typeid(Long_t))
            return "Long_t";
         if (typeid(T) == typeid(Float_t))
            return "Float_t";
         if (typeid(T) == typeid(Double_t))
            return "Double_t";
         if (typeid(T) == typeid(Bool_t))
            return "Bool_t";
         if (typeid(T) == typeid(TString))
            return "TString";
         if (typeid(T) == typeid(IntVec_t))
            return "IntVec_t";
         if (typeid(T) == typeid(FloatVec_t))
            return "FloatVec_t";
         if (typeid(T) == typeid(std::vector<Double_t>))
            return "vector<Double_t>";
         if (typeid(T) == typeid(std::vector<Bool_t>))
            return "vector<Bool_t>";
         if (typeid(T) == typeid(StringVec_t))
            return "StringVec_t";
         return "unsupported";
      }
      ...
      template <class T1>
      const T1 &GetRawValue() { return fParameter; }
