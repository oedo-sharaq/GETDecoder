...
void GETDecoder::Initialize()
{
  ...
  if (fMutantFrame == NULL)
    fMutantFrame = new GETMutantFrame();
  else
    fMutantFrame->Clear();
...
GETCoboFrame *GETDecoder::GetCoboFrame(Int_t frameID)
{
  ...
  while (kTRUE)
  {
    ...
    if (fIsDoneAnalyzing)
      if (fTargetFrameInfoIdx > fCoboFrameInfoArray->GetLast() || (fTargetFrameInfoIdx == fCoboFrameInfoArray->GetLast() && ((GETFrameInfo *)fCoboFrameInfoArray->Last())->GetNumFrames() != fTopologyFrame->GetAsadMask().count()))
        return NULL;
    ...
    while (fCoboFrameInfo->IsFill())
    {
      ...
      if (fCoboFrameInfo->GetNumFrames() == fTopologyFrame->GetAsadMask().count())
      {
        ...
        if (fCoboFrameInfoIdx == fTargetFrameInfoIdx)
        {
          ...
          fCoboFrameInfo = (GETFrameInfo *)fCoboFrameInfoArray->ConstructedAt(fCoboFrameInfoIdx);
          //std::cout<<"fCoboFrameInfo->GetDataID(): "<<fCoboFrameInfo->GetDataID()<<" fCurrentDataID: "<<fCurrentDataID<<std::endl;
...
#endif
          //if (fCoboFrame->GetFrame(0)->GetEventID() !=  fCoboFrameInfoIdx){
          //cout << "fCoboFrameInfoIdx: " << fCoboFrameInfoIdx << ", Event ID: " << fCoboFrame->GetFrame(0)->GetEventID() << ", fFrameInfoIdx:"<<fFrameInfoIdx << endl;
          //if (fCoboFrameInfoIdx != fCoboFrame->GetFrame(0)->GetEventID()){
          //  fCoboFrameInfo->Clear();
          //}
          //else{
            //if (fCoboFrameInfoIdx!=fCoboFrame->GetFrame(0)->GetEventID()){
            if (frameID==0){
            cout << "Returned fCoboFrameInfoIdx: " << fCoboFrameInfoIdx << " with event ID: " << fCoboFrame->GetFrame(0)->GetEventID() << ", fFrameInfoIdx:"<<fFrameInfoIdx << endl;
            }
            return fCoboFrame;
          //}
          //}
          //std::cout << "fCoboFrameInfo->GetEventID(): " << fCoboFrameInfo->GetEventID() << std::endl;
          //std::cout<<"Frame info returned successfully"<<std::endl;
    ...
    /*if (frameID>=1227382&&fFrameInfo->GetEventID()==fCoboFrameInfo->GetEventID()&&fCoboFrameInfoIdx==fTargetFrameInfoIdx&&fCoboFrameInfo->GetNumFrames()<4&&iter>1){
      startByte = fData.seekg(pos);
      std::cout<<"Starting from pos at frameinfoid: " << fFrameInfoIdx << std::endl;
    }
    else
    {*/
    ULong64_t startByte = fData.tellg();
    ...
    fFrameInfo->SetEndByte(endByte);
    fFrameInfo->SetEventID(fBasicFrameHeader->GetEventID());
    /*if (fFrameInfo->GetEventID()>=1227382){
      std::cout << " fFrameInfo->GetEventID(): " << fFrameInfo->GetEventID()<<" @fFrameInfoIdx: "<<fFrameInfoIdx <<std::endl;
    }*/
    /*if (iter<=2&&frameID>=1227384&&fFrameInfo->GetEventID()==fTargetFrameInfoIdx&&fCoboFrameInfo->GetNumFrames()<3){
      fBasicFrameHeader->Read(fData);
      if (fBasicFrameHeader->GetEventID()>fTargetFrameInfoIdx){
        fData.seekg(fFrameInfo->GetStartByte());
        cout<<"Reset at fCoboFrameInfo->GetNumFrames(): "<<fCoboFrameInfo->GetNumFrames()<<endl;

      }
    }*/
    /*if (frameID==1227382&&fFrameInfo->GetEventID()>fTargetFrameInfoIdx&&fCoboFrameInfo->GetNumFrames()==3){
      for (Int_t iback = 0; iback<fFrameInfoIdx; iback++){
        GETFrameInfo *checkFrameInfo = (GETFrameInfo *)fFrameInfoArray->ConstructedAt(iback);
        if (checkFrameInfo->GetEventID()==fTargetFrameInfoIdx){
               fCoboFrameInfo->SetNextInfo(checkFrameInfo);
               //cout << "go back in the stream to find fFrameInfoIdx: " << iback << endl;
               //cout << "after, fCoboFrameInfo->GetNumFrames(): " << fCoboFrameInfo->GetNumFrames() << endl;
               break;
        }
      }
    }*/
    //if (frameID>=1227382&&fFrameInfo->GetEventID()==fCoboFrameInfo->GetEventID()&&fCoboFrameInfoIdx==fTargetFrameInfoIdx&&fCoboFrameInfo->GetNumFrames()<3){
    if (frameID<=2){
      //std::cout << "fCoboFrameInfo->GetEventID(): " << fCoboFrameInfo->GetEventID() << " ,fFrameInfo->GetEventID(): " << fFrameInfo->GetEventID()<<" fBasicFrameHeader->GetEventID(): "<<fBasicFrameHeader->GetEventID() <<std::endl;
    }
    //std::cout << "fFrameInfo -> GetEndByte(): " << fFrameInfo -> GetEndByte() << " fData.tellg(): " << fData.tellg() << " fDataSize: " << fDataSize << std::endl;
    CheckEndOfData();
    if (fCoboFrameInfo->GetNumFrames() == 0) {//&& fFrameInfo->GetEventID() == fTargetFrameInfoIdx){
    //if (fCoboFrameInfo->GetNumFrames() == 0 && fFrameInfo->GetEventID() == fTargetFrameInfoIdx){
      fCoboFrameInfo->Copy(fFrameInfo);
    }
    else if (fCoboFrameInfo->GetEventID() == fFrameInfo->GetEventID() ){// && fFrameInfo->GetEventID() == fTargetFrameInfoIdx) {
    //else if (fCoboFrameInfo->GetEventID() == fFrameInfo->GetEventID() && fFrameInfo->GetEventID() == fTargetFrameInfoIdx) {

    //#ifdef DEBUG
    //std::cout << " fCoboFrameInfo->GetEventID(): " << fCoboFrameInfo->GetEventID() << " ,fFrameInfo->GetEventID(): " << fFrameInfo->GetEventID()<< std::endl;
    //#endif
    ...
    else
    {
      ...
      while (GETFrameInfo *checkCoboFrameInfo = (GETFrameInfo *)fCoboFrameInfoArray->ConstructedAt(iChecker))
      {
        ...
        {
          #ifdef DEBUG
            std::cout << "checkCoboFrameInfo->GetEventID(): " << checkCoboFrameInfo->GetEventID() << " ,fFrameInfo->GetEventID(): " << fFrameInfo->GetEventID()<< std::endl;
            #endif
          if (checkCoboFrameInfo->GetEventID() == fFrameInfo->GetEventID())
          //if (checkCoboFrameInfo->GetEventID() == fFrameInfo->GetEventID() && fFrameInfo->GetEventID() == fTargetFrameInfoIdx)
          {
        ...
        {
          #ifdef DEBUG
          std::cout<< "iChecker: " << iChecker <<std::endl;
          #endif
          //if (frameID<=1227382){
      ...
      #ifdef DEBUG
      std::cout<<"fFrameInfo->GetEventID() after checking: " << fFrameInfo->GetEventID() <<std::endl;
      #endif
    }
    /*if (frameID<=2){
      iter++;
...
void GETDecoder::RestorePreviousState()
{
  ...
  if (fPrevDataID != fCurrentDataID)
    SetData(fPrevDataID);
    //std::cout << "-------------RestorePreviousState---------------" << std::endl;
